...
// at https://github.com/quartzjer/js0n
...
#define CAP(i)                                                                 \
  if (depth == 1) {                                                            \
    ...
    if (klen && start) {                                                       \
      index =                                                                  \
          (klen == (size_t)(cur - start) && strncmp(key, start, klen) == 0)    \
              ? 0                                                              \
              : 2;                                                             \
      start = 0;                                                               \
    }                                                                          \
...
// this makes a single pass across the json bytes, using each byte as an index
// into a jump table to build an index and transition state
  ...
  again:
    ...
    case S_STRUCT:
      ...
      case '-':
        goto l_bare;
      default: {
        if (range(*cur, '0', '9') || range(*cur, 'A', 'Z') ||
            range(*cur, 'a', 'z'))
          goto l_bare;
        else
          goto l_bad;
