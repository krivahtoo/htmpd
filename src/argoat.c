...
void argoat_unflagged_sacrifice(const struct argoat *args) {
  args->sprigs[0].func(args->sprigs[0].data, args->unflagged,
...
// function execution
void argoat_sacrifice(struct argoat *args, char *flag, char **pars,
                      int pars_count) {
...
void argoat_graze(struct argoat *args, int argc, char **argv) {
  ...
  for (int i = 0; i < argc; ++i) {
    ...
    // pars
    if (argv[i][0] != '-') {
      pars_count += argoat_increment_pars(args, flag, argv[i]);
      // lone dash pars
    } else if (dash == '\0') {
      pars_count += argoat_increment_pars(args, flag, argv[i]);
